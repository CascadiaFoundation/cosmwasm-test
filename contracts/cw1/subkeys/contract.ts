...
export const CW1Subkeys = (keplrClient: SigningKeplrCosmWasmClient, txSigner: string): CW1SubkeysContract => {
  const use = (contractAddress: string): CW1SubkeysInstance => {
    ...
    const admins = async () => {
      return keplrClient.client.queryContractSmart(contractAddress, { admin_list: {} }) as Promise<AdminListResponse>
    }

    const freeze = async (senderAddress: string): Promise<string> => {
      const result = await keplrClient.client.execute(senderAddress, contractAddress, { freeze: {} }, 'auto')
      return result.transactionHash
    }

    // eslint-disable-next-line @typescript-eslint/no-shadow
    const updateAdmins = async (senderAddress: string, admins: readonly string[]): Promise<string> => {
      const result = await keplrClient.client.execute(senderAddress, contractAddress, { update_admins: { admins } }, 'auto')
      return result.transactionHash
    }

    const execute = async (senderAddress: string, msgs: readonly CosmosMsg[]): Promise<string> => {
      const result = await keplrClient.client.execute(senderAddress, contractAddress, { execute: { msgs } }, 'auto')
      return result.transactionHash
    }
