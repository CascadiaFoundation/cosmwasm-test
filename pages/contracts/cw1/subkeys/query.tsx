  ...
  const { data: response } = useQuery(
    ...
    async ({ queryKey }) => {
      ...
      const _canExecuteMessage = () => {
        switch (executeType) {
          case 'send':
            return `{"bank": {"send": {"to_address": "${_toAddressState.value}", "amount": [{"amount": "${_amountState.value}", "denom": "ucascadiadx"}]}}}`
          case 'withdraw':
            return `{"distribution": {"set_withdraw_address": {"address": "${_toAddressState.value}"}}}`
          case 'redelegate':
            return `{"staking": {"${_executeType}": {"src_validator": "${_validatorState.value}+'","dst_validator": "${_dstValidatorState.value}","amount": {"amount":"${_amountState.value}", "denom": "ucascadiadx"}}}}`
          default:
            return `{"staking": {"${_executeType}": {"validator": "${_validatorState.value}","amount": {"amount":"${_amountState.value}", "denom": "ucascadiadx"}}}}`
        }
      }
