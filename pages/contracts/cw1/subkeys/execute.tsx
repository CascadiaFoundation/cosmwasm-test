...
const CW1SubkeysExecutePage: NextPage = () => {
  ...
  const messageState = () => {
    switch (executeType) {
      case 'send':
        return `{"bank": {"send": {"to_address": "${recipientState.value}", "amount": [{"amount": "${amountState.value}", "denom": "ucascadiadx"}]}}}`
      case 'withdraw':
        return `{"distribution": {"set_withdraw_address": {"address": "${recipientState.value}"}}}`
      case 'redelegate':
        return `{"staking": {"redelegate": {"src_validator": "${validatorState.value}","dst_validator": "${dstValidatorState.value}","amount": {"amount":"${amountState.value}", "denom": "ucascadiadx"}}}}`
      case 'delegate':
      case 'undelegate':
        return `{"staking": {"${executeType}": {"validator": "${validatorState.value}","amount": {"amount":"${amountState.value}", "denom": "ucascadiadx"}}}}`
      default:
        return ``
    }
  }
