...
export const isValidAccountsFile = (file: AccountProps[]) => {
  ...
  const checks = file.map((account) => {
    ...
    // Check if address start with cascadiad
    if (!account.address.startsWith('cascadiad') && !account.address.startsWith('terra')) {
      return { address: false }
    }
  ...
  const iscascadiadAddresses = file.every((account) => account.address.startsWith('cascadiad'))
  const isTerraAddresses = file.every((account) => account.address.startsWith('terra'))
  if (!iscascadiadAddresses && !isTerraAddresses) {
